  ...
  function makeParticles(season: Season, width: number): Particle[] {
    ...
    return Array.from({ length: count }, (_, index) => {
      ...
      return {
        ...
        gravity: season === 'summer' ? randomBetween(150, 235) : 0,
        bounceCount: 0,
  ...
  onMount(() => {
    ...
    function updateSummerParticle(particle: Particle, delta: number, nowSeconds: number) {
      ...
      if (particle.x - radius <= 0 && particle.velocityX < 0) {
        particle.x = radius;
        particle.velocityX = Math.abs(particle.velocityX) * randomBetween(0.76, 0.9);
        bounced = true;
      ...
      if (particle.y + radius >= height && particle.velocityY > 0) {
        ...
        particle.velocityY = -Math.abs(particle.velocityY) * randomBetween(0.58, 0.74);
        particle.velocityX += randomBetween(-90, 90);
        bounced = true;
      }

      if (bounced) {
        particle.bounceCount += 1;
        particle.spin += particle.velocityX * 0.008 + randomBetween(-0.7, 0.7);
        particle.spin = Math.max(-5.2, Math.min(5.2, particle.spin));
