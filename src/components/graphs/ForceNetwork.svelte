<script lang="ts">
  ...
  const nodeRadius = (node: NetworkNode) =>
    node.radius ?? (node.featured ? 72 : 43);
  ...
  function configureForces() {
    ...
    const xForce = forceX<SimulationNode>((node) => node.anchorX).strength(
      (node) =>
        node.id === centerNodeId
    ...
    const yForce = forceY<SimulationNode>((node) => node.anchorY).strength(
      (node) =>
        node.id === centerNodeId
          ? config.centerAnchorStrength
          : config.anchorStrength,
