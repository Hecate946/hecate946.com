"""Unique Blender additions for the red room.
...
def add_static(context):
    """Add permanent red-room objects that are baked into the panorama.

    Example shared chair placement::

        context.place_static_asset(
            "chair",
            name="RedReadingChair",
...
def add_interactive(context):
    """Add browser-side red-room objects to the interactive GLB.

    Example movable shared chair placement::

        context.place_interactive_asset(
            "chair",
            name="RedMovableChair",
            grabbable=True,
