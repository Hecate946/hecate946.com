...
def add_static(context):
    """Add permanent purple-room objects that are baked into the panorama.

    Example shared chair placement::

        context.place_static_asset(
            "chair",
            name="PurpleReadingChair",
...
def add_interactive(context):
    """Add browser-side purple-room objects to the interactive GLB.

    Example movable shared chair placement::

        context.place_interactive_asset(
            "chair",
            name="PurpleMovableChair",
            grabbable=True,
